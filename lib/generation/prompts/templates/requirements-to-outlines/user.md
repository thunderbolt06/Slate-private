...
1. **quiz scenes must include quizConfig**:
   ...
   ```
   Aim for **5–8 questions** per quiz, with a **mix of question types** (e.g. single-choice, multiple-choice, true/false, fill-in-the-blank) and a mix of difficulties so each quiz feels substantial and tests recall, application, and understanding.
...
5. **Quiz placement**: NEVER place a quiz before the learner has been taught the concepts being tested. The first quiz scene MUST come after at least 3 concept/slide scenes have introduced the underlying material. After that, insert a quiz roughly every 3-5 slides for assessment, and only over content that has already been covered earlier in the course.
